...
export const PLChart = memo(function PLChart({ ticker }: PLChartProps) {
  ...
  // Normalize strategy type once and use everywhere consistently
  ...
  const generatePLData = (isExpandedView: boolean = false) => {
    ...
    // Normalize strategy type to handle any casing/naming inconsistencies (already normalized above)
    const normalized = normalizedStrategyType;
    ...
    const strategyParams = getStrategyParameters(normalized);
    ...
    if (isExpandedView && normalized === 'short_strangle') {
  ...
  // Get strategy parameters for display (use normalized)
  ...
  const ChartComponent = ({ data: chartData, isModal = false }: { data: any[], isModal?: boolean }) => {
    ...
    if (isModal && normalizedStrategyType === 'short_strangle') {
      // Use adaptive ±25 range for short strangle expanded view, but ensure it's proportional to stock price
      const adaptiveWindow = Math.max(25, currentPriceNum * 0.08); // At least ±25 or ±8% of stock price
      priceWindow = Math.min(adaptiveWindow, currentPriceNum * 0.4); // Cap at 40% to avoid extreme ranges
    } else {
        ...
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={chartData} margin={{ top: 5, right: 5, left: 5, bottom: 5 }}>
            ...
            <YAxis 
              ...
              axisLine={false}
              tickLine={false}
              domain={normalizedStrategyType === 'short_strangle' 
        ...
        <DialogContent className="max-w-6xl w-[90vw] h-[80vh] p-0 flex flex-col">
          <DialogHeader className="px-6 py-4 border-b">
            <DialogTitle className="text-xl font-semibold flex items-center gap-3">
              {ticker.symbol} {normalizedStrategyType.replace('_', ' ').toUpperCase()} P&L Chart
